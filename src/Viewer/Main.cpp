...
int main() {
  ...
  GLFWwindow *window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, u8"Í¼ÐÎÒýÇæ", nullptr, nullptr);


  GLFWimage images[1];
  //images[0].pixels = stbi_load("../../icon.png", &images[0].width, &images[0].height, 0, 4); // 4 channels for RGBA
  images[0].pixels = stbi_load("../icon.png", &images[0].width, &images[0].height, 0, 4); // 4 channels for RGBA
  if (!images[0].pixels) {
