...
class ConfigPanel {
 public:
  ...
  inline void setUpdateBiasFunc(const std::function<void(float &)> &func) {
      updateBiasFunc_ = func;
  }
 ...
 private:
  ...
  float lightPositionAngle_ = glm::radians(180.f);
  float biasPercent = 0.0f;
  ...
  std::function<void(float & bias )> updateBiasFunc_;
